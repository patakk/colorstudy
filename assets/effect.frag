...
void main() {
    ...
    vec4 mask = texture2D(tex2, uv) * u_usemask + vec4(1.) * (1.-u_usemask);
    vec4 maskb = mask;
    if(maskb.r > 0.0){
        maskb.rgb = vec3(1.);
    }
    ...
    vec2 uvrd = uv - 2.61*ff*vec2(1., 0.) + 0.*333.5*ff*vec2(1., 0.)/u_resolution*2.;
    vec2 uvgd = uv - 2.61*ff*vec2(1., 0.) + 0.*ff*vec2(1., 0.)/u_resolution*2.;
    vec2 uvbd = uv - 2.61*ff*vec2(1., 0.) - 0.*333.5*ff*vec2(1., 0.)/u_resolution*2.;
    ...
    if(uv.x < marg1 || uv.x > 1.-marg1 || uv.y < marg1 || uv.y > 1.-marg1){
        outc = vec4(.1);
    }
    if(hasmargin > 0.01){
